...
| LD-14 | Composable block structure — pages are visual assemblies of discrete, self-contained sections; each block has its own background, padding, and clear edges; structure is derived from content architecture (not imposed as decoration); blocks are full-width vertical sections, never horizontal card grids; reading direction stays linear. Constraint: block structure that tips into landing-page card grid violates this decision — composability is in the assembly, not the layout matrix. Resonates with all three brief tensions without naming them: EA thinks in composed systems; Agile works in discrete increments; coaching operates session by session, each complete in itself. Distinguished from LD-04 (metaphor-free): this is structural honesty, not metaphor — the visual structure reflects what the content actually is. **Amendment (2026-05-14):** blocks may use a 2-column image+text layout (33/66 ratio, image/text) — this is one content item with visual accompaniment, not a card grid; the reading direction within the block remains linear. Image placement is explicit via frontmatter `image_position: left\|right`; `auto` (alternates opposite to preceding image block) is reserved for future implementation. Blocks without an image degrade to single-column. On mobile, image stacks above text regardless of left/right position. Images must be content-honest per LD-04 — no purely decorative imagery at block level; representative diagrams (CLD-style, system maps) are in scope | Yes — if implementation reveals the distinction cannot be held in practice | Stability test — emerged from use, 2026-05-14 |
