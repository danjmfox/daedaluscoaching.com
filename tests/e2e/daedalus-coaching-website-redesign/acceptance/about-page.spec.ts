...
test.skip(
  "visitor reads the about page and sees the narrative prose from content blocks",
  async ({ page }) => {
    await page.goto("/about");

    // The composable resolves block slugs and renders their body HTML.
    // The spike confirmed both block bodies appear as text nodes in the page.
    // This text must come from a real block document, not the page frontmatter.
    await expect(
      page.getByText("This is the about intro block prose", { exact: false }),
    ).toBeVisible();
  },
);
...
test.skip(
  "visitor sees all content blocks rendered in the order declared in the page frontmatter",
  async ({ page }) => {
    await page.goto("/about");
...
test.skip(
  "visitor navigating to an unknown about path sees a not-found response",
  async ({ page }) => {
