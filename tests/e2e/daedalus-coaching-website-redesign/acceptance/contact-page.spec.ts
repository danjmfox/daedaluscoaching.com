...
test.skip(
  "the contact form is marked for Netlify Forms processing",
  async ({ page }) => {
    await page.goto("/contact");
...
test.skip(
  "visitor submits a valid enquiry and receives a confirmation message",
  async ({ page }) => {
    await page.goto("/contact");
    ...
    await expect(page.getByText(/thank you|message sent|we.ll be in touch/i)).toBeVisible();
  },
);

// Scenario 4 (error path): visitor submits without filling any fields.
test.skip(
  "visitor submitting an empty form sees a required-field error",
  async ({ page }) => {
    await page.goto("/contact");
    ...
    await expect(page.getByText(/required|please enter|cannot be blank/i)).toBeVisible();
  },
);

// Scenario 5 (error path): visitor enters a malformed email address.
test.skip(
  "visitor entering a malformed email address sees an email format error",
  async ({ page }) => {
    await page.goto("/contact");
...
test.skip(
  "visitor submitting without a name sees a name-required error",
  async ({ page }) => {
    await page.goto("/contact");
...
test.skip(
  "visitor submitting without a message sees a message-required error",
  async ({ page }) => {
    await page.goto("/contact");
